 ...
 * @returns {Promise<number>} resolves with the created restore point ID
 */
const createRestorePoint = (vm, title, type) => {
    // Snapshot the VM before opening IndexedDB. Opening the database may yield
    // for long enough that a Blockly drag finishes or another UI callback
    // mutates the project. A restore point must describe the exact state at
    // the call boundary, not whichever transient frame exists when storage is
    // finally ready.
    let projectFiles;
    try {
        projectFiles = vm.saveProjectSb3DontZip();
    } catch (error) {
    ...
    return openDB().then(db => new Promise((resolveTransaction, rejectTransaction) => {
...
/**
 * Read one asset owned by a specific restore point without loading that
 * project into the VM. Studio uses this to reproduce file-import gestures
 * whose source bytes are no longer present at the earlier history boundary.
 *
 * @param {number} id restore point ID
 * @param {string} md5ext content-addressed asset key
 * @returns {Promise<Uint8Array>} exact stored asset bytes
 */
const getRestorePointAsset = (id, md5ext) => openDB().then(db => new Promise((resolve, reject) => {
    const transaction = db.transaction([METADATA_STORE, ASSET_STORE], 'readonly');
    transaction.onerror = event => {
        reject(new Error(`Getting restore point asset: ${event.target.error}`));
    };

    const metadataRequest = transaction.objectStore(METADATA_STORE).get(id);
    metadataRequest.onsuccess = () => {
        const metadata = metadataRequest.result && parseMetadata(metadataRequest.result);
        if (!metadata || !Object.prototype.hasOwnProperty.call(metadata.assets, md5ext)) {
            reject(new Error(`Restore point ${id} does not own asset ${md5ext}`));
            return;
        }
        const assetRequest = transaction.objectStore(ASSET_STORE).get(md5ext);
        assetRequest.onsuccess = () => {
            if (assetRequest.result) resolve(assetRequest.result);
            else reject(new Error(`Restore point asset is unavailable: ${md5ext}`));
        };
    };
}));
